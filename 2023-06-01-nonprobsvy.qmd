...
`nonprobsvy` (commit
`c7d1ed6dbfbdcdb0088ca7cf8bfe4092ca1e862c`).
...
remotes::install_github("ncn-foreigners/nonprobsvy@c7d1ed6dbfbdcdb0088ca7cf8bfe4092ca1e862c")
...
for (b in 1:B) {
  ...
  sample_b$w_b <- N/n_b
  res_y1 <- nonprob(outcome = y1 ~ x, data = sample_b, svydesign = svy_a)
  res_y1_ipw <- nonprob(target = ~y1, selection= ~ x, data = sample_b, svydesign = svy_a)
  res_y2 <- nonprob(outcome = y2 ~ x, data = sample_b, svydesign = svy_a)
  res_y2_ipw <- nonprob(target = ~y2, selection= ~ x, data = sample_b, svydesign = svy_a)
  res_y3 <- nonprob(outcome = y3 ~ x, data = sample_b, svydesign = svy_a)
  res_y3_ipw <- nonprob(target = ~y3, selection= ~ x, data = sample_b, svydesign = svy_a)
...
results_mi$relMSE <- results_mi$mse/results_mi$mse[c(7,8,9,7,8,9,7,8,9)]*100
