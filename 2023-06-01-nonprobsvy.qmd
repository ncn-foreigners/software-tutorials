...
res_y1_ps <- nonprob(selection = ~ x,
                     target = ~ y1,
                     data = sample_b,
                     svydesign = svy_a)

res_y2_ps <- nonprob(selection = ~ x,
                     target = ~ y3,
                     data = sample_b,
                     svydesign = svy_a)

res_y3_ps <- nonprob(selection = ~ x,
                     target = ~ y3,
                     data = sample_b,
                     svydesign = svy_a)
...
lapply(list("mi_y1"=res_y1, "mi_y2"=res_y2, "mi_y3"=res_y3), "[[", "confidence_interval") |>
  do.call("rbind", args = _) |>
  transform(true = c(mean(y1), mean(y2), mean(y3)),
            naive = c(mean(sample_b$y1),mean(sample_b$y2), mean(sample_b$y3)),
...
set.seed(20230602)
B <- 1000
results <- matrix(data = 0, nrow = B, ncol = 6)
...
Bias, variance, mse and relative mse

```{r mi-sim-bias-reports}
results_mi <- data.frame(
  est = c("mi_y1", "mi_y2", "mi_y3", "naive_y1", "naive_y2", "naive_y3"),
  bias = colMeans(results) - c(colMeans(pop[, 2:4]),colMeans(pop[, 2:4])),
  var = apply(results, 2, var)*1000
) 

results_mi$mse <- with(results_mi, bias^2+var)
results_mi$relMSE <- results_mi$mse/results_mi$mse[c(4,5,6,4,5,6)]*100
results_mi 
```
