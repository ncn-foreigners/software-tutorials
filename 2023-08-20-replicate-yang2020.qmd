...
Now, we provide a single run. First, we generate probability sample and calibrate to known population totals from `X_totals`.
...
est_ipw <- nonprob(selection = X_formula,
...
est_dr_pred_y11 <- nonprob(outcome = as.formula(paste("Y_11 ~ ", 
                                                      paste(names(est_mi_y11$parameters[,1])[-1],
                                                            collapse = "+"))),
...
est_dr_pred_y12 <- nonprob(outcome = as.formula(paste("Y_12 ~ ", 
                                                      paste(names(est_mi_y12$parameters[,1])[-1],
                                                            collapse = "+"))),
                           selection = as.formula(paste(" ~ ", 
                                                      paste(names(est_mi_y12$parameters[,1])[-1],
                                                            collapse = "+"))),
...
est_dr_pred_y21 <- nonprob(outcome = as.formula(paste("Y_21 ~ ", 
                                                      paste(names(est_mi_y21$parameters[,1])[-1],
                                                            collapse = "+"))),
                           selection = as.formula(paste(" ~ ", 
                                                      paste(names(est_mi_y21$parameters[,1])[-1],
                                                            collapse = "+"))),
...
est_dr_pred_y22 <- nonprob(outcome = as.formula(paste("Y_22 ~ ", 
                                                      paste(names(est_mi_y22$parameters[,1])[-1],
                                                            collapse = "+"))),
                           selection = as.formula(paste(" ~ ", 
                                                      paste(names(est_mi_y22$parameters[,1])[-1],
                                                            collapse = "+"))),
...
est_dr_mm_y11 <- nonprob(outcome = as.formula(paste("Y_11 ~ ", paste(rownames(est_dr_y11$parameters)[-1], collapse = "+"))),
                         selection = as.formula(paste(" ~ ", paste(rownames(est_dr_y11$parameters)[-1], collapse = "+"))),
                         data = sample_B1,
                         svydesign = sample_A_svy,
                         control_selection = controlSel(est_method_sel = "mm"),
                         family_outcome = "gaussian")
est_dr_mm_y12 <- nonprob(outcome = as.formula(paste("Y_12 ~ ", paste(rownames(est_dr_y12$parameters)[-1], collapse = "+"))),
                         selection = as.formula(paste(" ~ ", paste(rownames(est_dr_y12$parameters)[-1], collapse = "+"))),
                         data = sample_B1,
                         svydesign = sample_A_svy,
                         control_selection = controlSel(est_method_sel = "mm"),
                         family_outcome = "gaussian")
est_dr_mm_y21 <- nonprob(outcome = as.formula(paste("Y_21 ~ ", paste(rownames(est_dr_y21$parameters)[-1], collapse = "+"))),
                         selection = as.formula(paste(" ~ ", paste(rownames(est_dr_y21$parameters)[-1], collapse = "+"))),
                         data = sample_B1,
                         svydesign = sample_A_svy,
                         control_selection = controlSel(est_method_sel = "mm"),
                         family_outcome = "binomial")
est_dr_mm_y22 <- nonprob(outcome = as.formula(paste("Y_22 ~ ", paste(rownames(est_dr_y22$parameters)[-1], collapse = "+"))),
                         selection = as.formula(paste(" ~ ", paste(rownames(est_dr_y22$parameters)[-1], collapse = "+"))),
                         data = sample_B1,
                         svydesign = sample_A_svy,
                         control_selection = controlSel(est_method_sel = "mm"),
                         family_outcome = "binomial")
...
```{r, eval = FALSE}
results <- rbind(
  cbind(est_ipw$output,est_ipw$confidence_interval),
