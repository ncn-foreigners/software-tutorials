...
```{r}
ipw_logit_svydes <- nonprob(selection = ~ X1 + X2 + X3 + X4,
                               target = ~ Y,
                               data = sample_B,
                               svydesign = sample_A_svy_cal,
                               method_selection = "logit")

cbind(ipw_logit_svydes$output,ipw_logit_svydes$confidence_interval)
```
...
First, we start with IPW calibrated to population totals using `mle` with calibration constraints.
...
m1b <- nonprob(selection = ~ X1+X2+X3+X4,
              ...
              data = sample_B,
              pop_totals = X_totals[1:5], 
              control_selection = controlSel(optimizer = "optim", est_method_sel = "gee", h=1)) 
summary(m1b)
```
It may happen that instead of `pop_totals` we know population means but population size should be known
...
By default we use analytical variance estimator but it possible to use bootstrap

```{r}
m1d <- nonprob(selection = ~ X1+X2+X3+X4,
              target = ~ Y,
              data = sample_B,
              pop_totals = X_totals[1:5],
              method_selection = "logit", 
...
Now we use mass imputation under mis-specified model when only population totals are known.

```{r}
m2 <- nonprob(outcome = Y ~ X3 + X4 + X5 + X6,
              data = sample_B,
              pop_totals = X_totals[1:5],
              method_outcome = "glm",
...
And finaly, condider DR model when only population totals are known

```{r}
m3 <- nonprob(selection = ~ X1 + X2 + X3 + X4,
              outcome = Y ~ X3 + X4 + X5 + X6,
              data = sample_B,
              pop_totals = X_totals,
              method_outcome = "glm",
              family_outcome = "gaussian")
...
We can do variable selection

```{r, eval = F, echo=T}
m1 <- nonprob(selection = ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X12+X13+X14+X15+X16+X17+X18+X19+X20,
              target = ~ Y,
              data = sample_B,
              pop_totals = X_totals[1:21],
              #svydesign = sample_A_svy_cal,
              method_selection = "logit",
              control_inference = controlInf(vars_selection = TRUE),
...
```{r}
m3 <- nonprob(selection = ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X12+X13+X14+X15+X16+X17+X18+X19+X20,
              outcome = Y ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X12+X13+X14+X15+X16+X17+X18+X19+X20,
              data = sample_B,
              pop_totals = X_totals[1:21],
              method_outcome = "glm",
              family_outcome = "gaussian",
